...
public:
  explicit SimpleActionServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions()) : Node("simple_action_server", options)
  {
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Action server 'fibonacci' started.");
  }
...
private:
  ...
  {
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Received goal request with order: ", goal->order);
        return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
  }

  void acceptedCallback(std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::Fibonacci>> goal_handle)
  {
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::Fibonacci>> goal_handle)
  {
        ...
        for (int i=1; (i < goal->order) && rclcpp::ok(); ++i)
        {
            ...
            RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Published feedback: ");
            for (auto number : sequence) {
                RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "%d ", number);
            }
            loop_rate.sleep();
        }
